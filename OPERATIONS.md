...
**バイアスの規模(2026-08-14測定)**: 過去6年間(2020-08-14〜2026-08-14)でプライム市場から
実際に上場廃止になった銘柄は157件、現在のプライム市場銘柄数は1552件。つまりこの期間の
どこかの時点では、今のバックテストが見ていない銘柄が常に9〜10%程度存在していたことになる。
**この節で述べている限界はバックテストの数字そのものには一切反映していない**(修正不能の
ため)。これまで・これから報告するバックテストの数値(リターン・SQN等)は、常にこの
バイアスがかかったままの生の計算結果であり、実態よりやや楽観的である前提で読むこと。

なお、J-Quants API(JPXの個人開発者向けデータAPI)も調査したが、無料プランはデータ提供期間が
直近12週間を除く過去2年分に限られ(現在の検証は3〜6年分を使用)、上場廃止銘柄の株価データが
含まれるかも公式ドキュメントに明記が無く未確認。仮に使えても効果は限定的と判断し、現時点では
採用していない。
